...
class ClauseCNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat((x1, x2, x3, x4), 1) # (batch, hidden_num) 
        x = self.dropout(x)
        return x
...
class BiLSTM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        text = self.dropout(self.embed(text_indices)) # batch_size x doc_len x seq_len x embed_dim
        ...
        clause_rep = torch.cat(clause_reps, dim=1) # batch_size x doc_len x hidden_dim
        out, (_, _) = self.bilstm(clause_rep, doc_len) 
        emotion_rep = F.relu(self.emotion_fc(out))
        cause_rep = F.relu(self.cause_fc(out))
        # emotion_rep = F.leaky_relu(self.emotion_fc(out), 0.1)
        # cause_rep = F.leaky_relu(self.cause_fc(out), 0.1)
